...
<script>
import { getProCommons, getProDeail } from '@/api/product'
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      const { data: { detail } } = await getProDeail(this.goodsId)
      this.detail = detail
      this.images = detail.goods_images
    },
    async getComments () {
      const { data: { list, total } } = await getProCommons(this.goodsId, 3)
